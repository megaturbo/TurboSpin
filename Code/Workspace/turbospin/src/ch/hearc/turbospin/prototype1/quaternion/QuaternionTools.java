...
final public class QuaternionTools {
	...
	public static double getAngle(Quaternion q) {
		return Math.acos(q.getReal()) * 2.0;
	}

	public static Vector3D getAxis(Quaternion q) {
		Vector3D tmp = q.getVector();
		return tmp.multiply(2.0 / Math.sin(getAngle(q)));
	}

	/**
	 * Creates a Quaternion containing the rotation of theta around the axis
	 * given
	 * 
	 * @param theta
	 * @param axisVector
	 * @return
	 */
	public static Quaternion createRotationQuaternion(double theta,
			Vector3D axisVector) {
	...
	public static Point3D rotation(Point3D point, Quaternion rot) {
		Vector3D output = rotation(new Vector3D(point.getX(), point.getY(),
				point.getZ()), rot);
	 ...
	 * @param axisVector
	 */
	public static Point3D rotation(Point3D point, double theta,
			Vector3D axisVector) {
		Quaternion q = QuaternionTools.createRotationQuaternion(theta,
				axisVector);
	 ...
	 * @param axisVector
	 */
	public static Vector3D rotation(Vector3D vector, double theta,
			Vector3D axisVector) {
		Quaternion q = QuaternionTools.createRotationQuaternion(theta,
				axisVector);
	 ...
	 * @param axisVector
	 */
	public static Vertex3D rotation(Vertex3D vertex, double theta,
			Vector3D axisVector) {
		Quaternion q = QuaternionTools.createRotationQuaternion(theta,
				axisVector);
